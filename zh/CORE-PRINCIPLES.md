...
此原则来自 2026-01-31 的 Task Decomposition 验证实践。在设计用户认证模块的 Claude Code prompts 时，最初的设计假设用户会手动执行技术步骤。经过 review，改为 AI 主导执行、用户只提供必要输入的模式，效果显著提升。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.0 | 2026-01-31 | 初始版本，包含 3 个核心原则 |
| v1.1 | 2026-02-01 | 新增原则 4：输入验证前置（基于 Agent 自动化实验） |
...
此原则来自 2026-02-01 的 Agent 自动化实验。用户输错了 MySQL 密码，Agent 将错误密码填充到所有 Prompt 模板中，直到第 6 步初始化脚本才发现连接失败，导致需要手动修复 `.env` 文件并重新执行。加入连接测试后，错误在输入阶段即可发现和修正。
