...
> 📄 模板：`templates/doc-dependencies-template.md`  
> 📄 检查清单：`templates/change-review-checklist-template.md`
...
**依赖人类记忆来追踪这些依赖关系是不可靠的。** 随着项目增长，文档不一致的风险会增加。

### 解决方案：文档依赖关系图

创建 `doc-dependencies.yaml` 来映射文档之间的关系：

```yaml
ui_prototype:
  triggers_review:
    - api-design.md      # UI 新功能 → 检查 API 是否存在
    - state-machines.md  # UI 新状态 → 检查状态机
    - business-rules.md  # UI 新逻辑 → 检查业务规则
    - prd.md             # UI 新功能 → 更新 PRD
```

### 工作流程

```
文档变更 → 查阅依赖关系图 → 检查受影响文档 → 全部更新 → 记录到控制文档
```
...
| 实践 | 说明 |
|------|------|
| **提交前检查** | 在提交变更前执行依赖检查清单 |
| **原子更新** | 在同一次提交中更新所有相关文档 |
| **版本对齐** | 保持相关文档版本同步 |
| **定期审计** | 定期检查文档是否存在漂移 |
...
| 触发条件 | 执行操作 |
|----------|----------|
| UI 原型变更后 | 检查 API、状态机、业务规则、PRD |
| PRD 变更后 | 检查所有技术文档 |
| API 变更后 | 检查数据库、业务规则、状态机 |
| 任何重大变更后 | 执行变更检查清单 |

### 产出物

- [ ] 文档依赖关系图 (`doc-dependencies.yaml`)
- [ ] 完成的变更检查清单（按需）
- [ ] 更新的项目控制文档
...
| v1.2 | 2025-01-28 | 添加文档一致性管理(Phase 8) |
