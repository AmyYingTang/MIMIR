...
> **Must be executed BEFORE the Completion Report** — otherwise the Agent will stop after outputting the report and skip these steps.
...
These 14 principles were extracted from real project execution experience. Apply them as a checklist when writing or reviewing prompts.
...
| 12 | **InlineAPIContract** | Prompts must **embed exact API request/response JSON schemas inline**, not just say "refer to api-design.md". Agents drift from referenced doc details (field names, paths, nesting) when generating large code volumes; inline schemas are the only reliable fidelity mechanism. **API schemas must include not only field names and nesting but also parameter validation constraints** (`min_length`, `max`, `le`, enum ranges, etc.) — otherwise frontend will hardcode illegal values | Example 1: api-design.md defines `{chip_id, chip_model, available_functions: [{function_type, function_name}]}`, prompt just says "refer to api-design.md". Agent generates `{id, model, functions: ["KWS"]}` → frontend field parsing fails entirely. Example 2: Frontend hardcodes `page_size: 1000` to fetch all at once, but backend Query param has `le=100`, returns 422. Frontend password min=6 vs backend min=8, username max=32 vs backend max=50, function types hardcoded 3/6 kinds — all the same pattern: prompt didn't inline parameter constraints, frontend guessed its own values |
...
| 14 | **ErrorCodeFidelity** | Backend global exception handlers must preserve HTTP status code granularity — 422 (parameter validation failure) and 401 (authentication failure) are entirely different problems and must not be mapped to the same business error code. Prompts involving exception handling logic should explicitly list the mapping rules for each HTTP status code | Example: FastAPI's `RequestValidationError` (422) was mapped by the global exception handler to 40100 ("not logged in or invalid token"). Debugging appeared to be a token issue, but the actual cause was `page_size=1000` exceeding the `le=100` validation limit. Root cause: Agent wrote exception handler with a catch-all that mapped all non-200 responses to a handful of error codes, losing error type information |
